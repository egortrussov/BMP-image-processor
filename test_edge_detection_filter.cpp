...
TEST_CASE("Test edge detection filter") {
    {
        static const std::string OUTPUT_FILE_PATH = "./test_images/shrek_edge_05.bmp";
        image_processor::Image image = image_processor::BMP::OpenImage("./test_images/shrek.bmp");
        image_processor::EdgeDetectionFilter edge_filter(0.5);  // NOLINT
        edge_filter.ApplyFilter(image);
    ...
    }
    {
        ...
        image_processor::EdgeDetectionFilter edge_filter(0.01);  // NOLINT
        edge_filter.ApplyFilter(image);
        REQUIRE(DoesImageDataMatch(image, IMAGES_DATA.at("small_image_edge")));
